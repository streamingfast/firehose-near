...
func init() {
	launcher.RegisterCommonFlags = func(_ *zap.Logger, cmd *cobra.Command) error {
		...
		cmd.Flags().IntSlice("common-block-index-sizes", []int{100000, 10000, 1000, 100}, "index bundle sizes that that are considered valid when looking for block indexes")
